...
import argparse
import pandas as pd
from pipeline_debug import load_tab_from_supabase
...
def calc_metrics(ticker: str, period: str) -> pd.DataFrame:
    ...
    print(f"DEBUG: calc_metrics({ticker}, {period}) started", flush=True)
    # Phase 4: Load sector-specific sheets
    from sector import SECTOR_SHEETS
    print(f"DEBUG: calling get_sector({ticker})", flush=True)
    ticker_sector = get_sector(ticker)
    print(f"DEBUG: sector for {ticker} is {ticker_sector}", flush=True)
    ...
    print(f"DEBUG: loading tabs from supabase...", flush=True)
    try:
        cdkt = load_tab_from_supabase(ticker, period, cdkt_sheet)
        print(f"DEBUG: loaded cdkt, rows={len(cdkt)}", flush=True)
        kqkd = load_tab_from_supabase(ticker, period, kqkd_sheet)
        print(f"DEBUG: loaded kqkd, rows={len(kqkd)}", flush=True)
        lctt = load_tab_from_supabase(ticker, period, lctt_sheet)
        print(f"DEBUG: loaded lctt, rows={len(lctt)}", flush=True)
