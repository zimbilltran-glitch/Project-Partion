...
def load_tab_from_supabase(ticker: str, period_type: str, sheet: str) -> pd.DataFrame:
    ...
    """
    print(f"DEBUG: load_tab_from_supabase({ticker}, {period_type}, {sheet})", flush=True)
    url = os.getenv("SUPABASE_URL")
    key = os.getenv("SUPABASE_KEY")
    if not url or not key:
    ...
    try:
        ...
        sb = get_sb()
        print("DEBUG: using shared supabase client", flush=True)
    except Exception as e:
        print(f"DEBUG: error getting shared client: {e}", flush=True)
        # Fallback to local import if needed
        from supabase import create_client
        sb = create_client(url, key)
    table_name = SHEET_TO_TABLE.get(sheet.lower())
    print(f"DEBUG: table_name={table_name} for sheet={sheet}", flush=True)
    if not table_name:
        raise ValueError(f"Invalid sheet: {sheet}")

    print(f"[{ticker}] Fetching {period_type} {table_name} from Supabase...", flush=True)
    try:
        data = []
        page_size = 1000
        for page in range(10):  # max 10k rows
            print(f"DEBUG: fetching page {page}...", flush=True)
            start = page * page_size
            end = start + page_size - 1
            print(f"DEBUG: calling sb.table({table_name}).select('*').eq('stock_name', {ticker}).range({start}, {end}).execute()", flush=True)
            response = sb.table(table_name).select("*").eq("stock_name", ticker).range(start, end).execute()
            print(f"DEBUG: query executed, received {len(response.data)} rows", flush=True)
